...
for rr_json in rr_files_json:
    with open(f'../reciprocal_rank/{rr_json}') as json_file:
        ...
        if data['status']['reciprocal_rank']['attempts_number'] == 1:
            mean_status_rr['attempts_number_only_1'] += 1
        
        if data['status']['reciprocal_rank']['attempts_number'] == 2:
            mean_status_rr['attempts_number_only_2'] += 1
